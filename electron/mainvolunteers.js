...
function cargarTabla() {
	...
	xhttp.open("GET", "http://localhost:3333/api/volunteers/all");
	xhttp.send();
	xhttp.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
			...
			for (let object of objects) {
				tablero += '<tr>';
				tablero += '<td>' + object['dnivoluntario'] + '</td>';
			...
			document.getElementById("tablavoluntarios").innerHTML = tablero;
		}
	};
}
...
btneditar.addEventListener('click', function(event) {
	...
	if (editdiv.style.display === "none") {
		...
		if (expresion_regular_dni.test(dni) != true) {
			...
			if (dni != "") {
				const xhttp = new XMLHttpRequest();
				xhttp.open("GET", "http://localhost:3333/api/volunteers/find/" + dni);
				xhttp.send();
				xhttp.onreadystatechange = function() {
					if (this.readyState == 4 && this.status == 200) {
						if (this.responseText == "") {
							...
							document.getElementById("txtdni").value = object['dnivoluntario'];
			...
			xhttp.onreadystatechange = function() {
				if (this.readyState == 4 && this.status == 200) {
					if (this.responseText == "") {
						...
						xhttp.open("DELETE", "http://localhost:3333/api/volunteers/delete/" + dni);
						xhttp.send();
						xhttp.onreadystatechange = function() {
							if (this.readyState == 4 && this.status == 200) {
								Swal.fire("El voluntario con DNI " + dni + " se ha eliminado correctamente");
								cargarTabla();
								deletediv.style.display = "none";
							}
						};
		...
		if ((dni == "") || (nom == "") || (ape == "") || (edad == "") || (fnac == "") || (fant == "") || (dir == "") || (tlf == "")) {
			...
			xhttp.onreadystatechange = function() {
				if (this.readyState == 4 && this.status == 200) {
					if (this.responseText != "") {
						const object = JSON.parse(this.responseText);
						xhttp.open("POST", "http://localhost:3333/api/volunteers/save");
						xhttp.setRequestHeader("Content-Type", "application/json;charset=UTF-8");
						xhttp.send(JSON.stringify({
							"idvoluntario": object['idvoluntario'], "dnivoluntario": dni, "nombre": nom, "apellidos": ape, "edad": edad,
						...
						xhttp.setRequestHeader("Content-Type", "application/json;charset=UTF-8");
						xhttp.send(JSON.stringify({
							"dnivoluntario": dni, "nombre": nom, "apellidos": ape, "edad": edad,
							"fecha_nac": fnac, "fecha_ant": fant, "direccion": dir, "telefono": tlf
						}));
					}
					xhttp.onreadystatechange = function() {
...
btnfind.addEventListener('click', function() {
	Swal.fire({
		title: 'Introduzca el DNI del voluntario',
		html:
		...
		preConfirm: () => {
			...
			if (expresion_regular_dni.test(dnivalue) != true) {
				...
				if (dnivalue != "") {
					xhttp.open("GET", "http://localhost:3333/api/volunteers/find/" + dnivalue);
					xhttp.send();
					xhttp.onreadystatechange = function() {
						if (this.readyState == 4 && this.status == 200) {
							var response = "";
							if (this.responseText != "") {
								const object = JSON.parse(this.responseText);
								response += "----- Resultado -----" + "\n";
								response += "DNI: " + object['dnivoluntario'] + "\n";
